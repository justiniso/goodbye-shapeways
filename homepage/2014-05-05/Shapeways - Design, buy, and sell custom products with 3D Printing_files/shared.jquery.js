...
/*
     FILE ARCHIVED ON 2:52:24 May 6, 2014 AND RETRIEVED FROM THE
...
(function ($) {
  ...
  $.swPortal.loadGooglePlus = function () {
    ...
    (function () {
      ...
      po.async = true;
      po.src = '/web/20140506025224/https://apis.google.com/js/plusone.js';
  ...
  SW.setSetting = function (setting, value) {
  ...
  SW.getSetting = function (setting, callback) {
 ...
 * /web/20140506025224/http://dev.twitter.com/pages/intents-events#click
...
function openLogInSignUpDialog(activeTab, callBack, getKey, internalReferrer) {
...
function fbLogIn(elem, showModal) {
        ...
        if (response.authResponse) {
          ...
          jQuery.ajax({
            ...
            success:function (swResponse) {
              return handleSSOLogInResponse(swResponse, elem, showModal);
            }
          });

        } else {
          // The person cancelled the login dialog; do nothing
        }
...
function handleSSOLogInResponse(response, elem, showModal) {
...
function disconnectSSO(ssoName) {

  jQuery.ajax({
    type:'get',
    data: { ssoName:  ssoName },
    dataType: "json",
    url:  '/user/disconnect-sso',
    success:function (response) {
  ...
  $(".passwordform_submit").live('click',  function() {
    submitCreatePassword(this);
  });

}

function submitCreatePassword(elem) {
  ...
  jQuery.ajax({
    ...
    success:function (response) {
     ...
     //if something bad display html
      if (response.success == true) {
        document.location.reload();
      } else if (response.success == false)  {
        var html = response.html;
        $(elem).parents('.fancybox-inner').html(html);
      }
    }
  });
