...
/*
     FILE ARCHIVED ON 10:38:04 May 24, 2013 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 1:40:16 Jun 27, 2015.
...
(function ($) {
  ...
  $.swPortal.loadGooglePlus = function () {
    ...
    (function () {
      ...
      po.async = true;
      po.src = '/web/20130524103804/https://apis.google.com/js/plusone.js';
 ...
 * /web/20130524103804/http://dev.twitter.com/pages/intents-events#click
...
function initDropDownMenus(container) {
  ...
  var $menuGroup = $(selector);

  $(selector).children('.dropdown-toggle').click(function(e) {
    var me = this;
    var $activeMenu = $('#' + $(this).attr('data-toggle'));

    $activeMenu.is(':visible') ? $menuGroup.removeClass('open') : $menuGroup.addClass('open');

    $(".dropdown-menu").filter(function(i, el) {
      return $(me).attr('data-toggle') != el.id;
    }).parent().removeClass('open');

    if($(this).parent().hasClass('center')) {

      var toggle_width = $(this).outerWidth();
      var menu_width = $(this).next('.dropdown-menu').outerWidth();
      var width = parseInt((menu_width - toggle_width)/2);
      $(this).next('.dropdown-menu').css('margin-left', -width + "px");
    };
    e.stopPropagation();
  });
